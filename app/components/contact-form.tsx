  ...
  return (
    <form className="contact-form" onSubmit={submit} noValidate>
      ...
      <div className="form-row">
        <label>Project type<select name="projectType" required defaultValue=""><option value="" disabled>Select</option>{contact.projectTypes.map((item) => <option key={item}>{item}</option>)}</select></label>
        <label>Budget range<select name="budgetRange" defaultValue=""><option value="">Not specified</option>{contact.budgetRanges.map((item) => <option key={item}>{item}</option>)}</select></label>
      </div>
      <label>Tell me about your project<textarea name="message" minLength={20} maxLength={2000} required rows={3} /></label>
      ...
      <button type="submit" disabled={state.status === "sending"}>
        {state.status === "sending" ? (
          <span className="ez-btn-indicator">
            <Image src="/assets/brand/logo/ez-symbol-black.webp" alt="" width={18} height={18} className="ez-btn-logo-icon" />
            Sending…
          </span>
        ) : (
          "Send inquiry ↗"
