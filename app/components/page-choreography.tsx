    ...
    const timer = setTimeout(() => {
      ctx = gsap.context(() => {
        ...
        if (pathname === "/") {
          ...
          // Home Portrait Reveal Clip (targets wrapper container div, not img)
          ...
          const toolsRing = document.querySelector(".ring-panel");
          if (toolsRing) {
            gsap.to(toolsRing, {
              rotation: 4,
              y: -15,
        ...
        if (pathname === "/about") {
          ...
          const ribbon = document.querySelector(".ribbon-artwork-container") || document.querySelector(".ribbon-panel");
          if (ribbon) {
            gsap.to(ribbon, {
              xPercent: -6,
              ease: "none",
              scrollTrigger: { trigger: ".practice-ribbon-wrapper", start: "top bottom", end: "bottom top", scrub: 1 },
            });
          }

          // Thinking Layers Separation & Alignment
          const thinkingLayers = document.querySelector(".layers-image-container") || document.querySelector(".layers-panel");
          if (thinkingLayers) {
            gsap.fromTo(
              thinkingLayers,
              { scale: 0.96, y: 20 },
              {
                ...
                y: 0,
                ease: "power2.out",
                scrollTrigger: { trigger: ".layers-artwork-wrapper", start: "top 85%", end: "center center", scrub: 0.6 },
              }
            );
          }
        }
        ...
        if (pathname === "/work") {
          ...
          const processPanel = document.querySelector(".process-stacked-artwork") || document.querySelector(".process-panel");
          if (processPanel) {
            gsap.fromTo(
              processPanel,
              { filter: "brightness(0.85) contrast(0.95)", y: 15 },
              {
                ...
                y: 0,
                ease: "power2.out",
                scrollTrigger: { trigger: ".process-stacked-artwork", start: "top 80%", end: "center center", scrub: 0.8 },
              }
            );
          }

          // Gyroscope Restrained 2.5D Rotation
          const gyroPanel = document.querySelector(".gyro-panel");
          if (gyroPanel) {
            gsap.to(gyroPanel, {
              rotation: -5,
              scale: 1.02,
        ...
        if (pathname === "/contact") {
          ...
          const calibration = document.querySelector(".calibration-panel");
          if (calibration) {
            gsap.to(calibration, {
              y: -12,
              ease: "none",
